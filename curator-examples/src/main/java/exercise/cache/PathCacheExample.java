 ...
 * cache of these changes and output when updates occurs.
 */
public class PathCacheExample {
        ...
        PathChildrenCacheListener listener = new PathChildrenCacheListener() {
            ...
            public void childEvent(CuratorFramework client, PathChildrenCacheEvent event) throws Exception {
                switch (event.getType())
                {
                    ...
                    case CHILD_REMOVED:
                    {
                        System.out.println("Node deleted: " + ZKPaths.getNodeFromPath( event.getData().getPath() ));
                        break;
                    }
    ...
    private static void setValue(CuratorFramework client, String command, String[] args) throws Exception {
        if (args.length != 2)
        {
            System.err.println( "syntax error (expected set <name> <value>): "+command );
            return;
        }
